-- Fix type casting issues in meeting_requests table
-- The error suggests there's a type mismatch between TEXT and UUID

-- First, let's check the current table structure
-- Then fix any type casting issues

-- Option 1: If the table has UUID columns but we're sending TEXT
-- We need to cast the TEXT to UUID in the insert

-- Option 2: If the table has TEXT columns but we're comparing with UUID
-- We need to cast UUID to TEXT in comparisons

-- Let's create a function to handle the insert with proper type casting
...
BEGIN
    ...
    -- Insert the meeting request with proper type casting
...
-- Also, let's make sure the can_make_meeting_request function handles type casting properly
-- Update it to ensure all comparisons use TEXT types
...
DECLARE
    ...
    remaining_boost DECIMAL(10,2);
BEGIN
    -- Check if user has an active pass (all TEXT comparisons)
    SELECT 
