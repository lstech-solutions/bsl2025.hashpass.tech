-- Setup Edward Calderon as a speaker with VIP access
-- This script will:
-- 1. Add Edward as a speaker in bsl_speakers table
-- 2. Update his pass to VIP access
-- 3. Ensure he can receive meeting requests
...
-- Step 2: Update Edward's pass to VIP access
-- First, let's check if he has a pass
DO $$
DECLARE
    edward_user_id uuid := '13e93d3b-0556-4f0d-a065-1f013019618b'::uuid;
    edward_pass_id text;
BEGIN
    -- Get Edward's pass ID
    SELECT id INTO edward_pass_id
    FROM public.passes
    WHERE user_id = edward_user_id
      AND event_id = 'bsl2025'
    LIMIT 1;
    
    IF edward_pass_id IS NOT NULL THEN
...
BEGIN
    ...
    WHERE mr.speaker_id::text = p_speaker_id
