-- ============================================================================
-- Function: can_make_meeting_request
-- Purpose: Check if a user can make a meeting request to a speaker
-- 
-- Parameters:
--   p_user_id (UUID): The requester's user ID
--   p_speaker_id (TEXT): The speaker's ID (can be UUID string or old slug)
--   p_boost_amount (DECIMAL): Optional boost amount
--
-- Returns: TABLE with can_request, reason, pass_type, remaining_requests, remaining_boost
-- ============================================================================
...
DECLARE
    ...
    v_speaker_uuid UUID;
BEGIN
    -- Convert p_speaker_id (TEXT) to UUID for comparison with user_blocks.speaker_id
    BEGIN
        -- Try to convert directly to UUID
    ...
    -- speaker_id in user_blocks is UUID, so compare with converted UUID
    IF v_speaker_uuid IS NOT NULL THEN
        SELECT EXISTS(
            SELECT 1 FROM user_blocks 
            WHERE speaker_id = v_speaker_uuid
