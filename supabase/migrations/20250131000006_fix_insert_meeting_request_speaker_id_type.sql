-- Fix insert_meeting_request to handle speaker_id type correctly
-- Handle both TEXT and UUID column types
...
BEGIN
    ...
    -- Determine speaker_id value based on column type
    -- Check if speaker has a user_id (UUID) in case the column is UUID
    SELECT user_id INTO speaker_user_id
    FROM public.bsl_speakers
    WHERE id = p_speaker_id;
    
    -- Try to insert with TEXT first (most common case)
    BEGIN
    ...
    EXCEPTION
        WHEN datatype_mismatch OR invalid_text_representation THEN
            -- Column is UUID, try using speaker's user_id if available
            IF speaker_user_id IS NOT NULL THEN
                INSERT INTO public.meeting_requests (
                    ...
                    new_request_id, 
                    requester_uuid, 
                    speaker_user_id,  -- Use UUID from user_id
                    p_speaker_name, 
                    p_requester_name,
                    p_requester_company, 
                    p_requester_title, 
            ...
            ELSE
                -- Speaker doesn't have user_id, can't create request
                result := json_build_object(
                    'success', false, 
                    'error', 'Speaker not linked to user account', 
