-- Fix insert_meeting_request to properly handle UUID speaker_id column
-- If column is UUID, we need to use speaker's user_id instead of speaker id
...
BEGIN
    ...
    -- Verify speaker exists and get user_id
    ...
    -- Determine what to insert based on column type
    -- Try TEXT first, if it fails with type error, use UUID (user_id)
    BEGIN
        -- Try inserting with TEXT speaker_id
    ...
    EXCEPTION
        WHEN datatype_mismatch OR invalid_text_representation THEN
            -- Column is UUID, use speaker's user_id
