-- Robust fix: Ensure speaker_user_id is definitely UUID before insert
-- Use SELECT ... INTO for INSERT to ensure type safety
...
BEGIN
    ...
    -- Explicitly cast to UUID to ensure type
    ...
    -- Ensure final UUID value is definitely UUID type
    speaker_uuid_final := speaker_user_id::UUID;
    
    -- Insert the meeting request with UUID speaker_id (user_id)
    -- Use INSERT ... SELECT to ensure type safety
    ...
    SELECT 
        new_request_id::UUID, 
        requester_uuid::UUID, 
        speaker_uuid_final::UUID,  -- Use the explicitly cast UUID variable
        p_speaker_name::TEXT, 
        p_requester_name::TEXT,
        COALESCE(p_requester_company, NULL)::TEXT, 
        COALESCE(p_requester_title, NULL)::TEXT, 
        p_requester_ticket_type::TEXT,
        p_meeting_type::TEXT, 
        p_message::TEXT, 
        COALESCE(p_note, NULL)::TEXT, 
