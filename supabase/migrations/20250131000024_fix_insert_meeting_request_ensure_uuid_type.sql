-- Fix insert_meeting_request to ensure speaker_user_id is definitely UUID before INSERT
-- Add extra validation step to catch any type issues before INSERT
...
BEGIN
    ...
    -- Step 10: Validate and cast speaker_user_id to UUID explicitly
    -- This ensures PostgreSQL recognizes it as UUID type
    BEGIN
    ...
    EXCEPTION
        WHEN OTHERS THEN
            result := json_build_object(
                'success', false, 
                'error', 'Invalid speaker user_id format', 
                'message', 'The speaker user_id is not a valid UUID: ' || COALESCE(speaker_user_id::TEXT, 'NULL')
            );
            RETURN result;
    END;
    
    -- Step 11: Insert the meeting request
    -- CRITICAL: meeting_requests.speaker_id is UUID, so we use speaker_user_id_validated (UUID)
    ...
    INSERT INTO public.meeting_requests (
        ...
        new_request_id,                    -- UUID
        requester_uuid,                    -- UUID
        speaker_user_id_validated,         -- UUID (VALIDATED and EXPLICITLY CAST)
        p_speaker_name,                     -- TEXT
        p_requester_name,                   -- TEXT
        p_requester_company,                -- TEXT
        p_requester_title,                  -- TEXT
