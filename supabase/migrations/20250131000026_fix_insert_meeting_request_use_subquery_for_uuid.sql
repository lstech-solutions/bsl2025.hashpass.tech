...
BEGIN
    ...
    -- Step 9: Validate speaker exists and has user_id
    IF NOT EXISTS (
        SELECT 1 FROM public.bsl_speakers 
        WHERE id = p_speaker_id AND user_id IS NOT NULL
    ) THEN
        result := json_build_object(
            'success', false, 
            'error', 'Speaker not found or not linked', 
            'message', 'The speaker does not exist or is not linked to a user account'
        );
        RETURN result;
    END IF;
    
    -- Step 10: Insert the meeting request using INSERT...SELECT with subquery
    -- CRITICAL: Use subquery to get speaker user_id directly as UUID in INSERT
    -- This forces PostgreSQL to recognize the UUID type correctly
    ...
    SELECT 
        new_request_id,                    -- UUID
        requester_uuid,                    -- UUID
        (SELECT user_id::UUID FROM public.bsl_speakers WHERE id = p_speaker_id),  -- UUID from subquery
        p_speaker_name,                     -- TEXT
        p_requester_name,                   -- TEXT
        p_requester_company,                -- TEXT
        p_requester_title,                  -- TEXT
    ...
    IF NOT FOUND THEN
        result := json_build_object(
            'success', false, 
            'error', 'Failed to insert meeting request', 
            'message', 'Could not create meeting request'
        );
        RETURN result;
    END IF;
