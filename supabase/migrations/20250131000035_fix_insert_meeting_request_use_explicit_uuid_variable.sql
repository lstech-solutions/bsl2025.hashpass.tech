-- Use explicitly typed UUID variable instead of subquery in SELECT
-- This ensures PostgreSQL correctly recognizes the UUID type
...
BEGIN
    ...
    -- Explicitly get and validate the UUID
    SELECT COALESCE(user_id, '00000000-0000-0000-0000-000000000000'::UUID)::UUID INTO speaker_user_id
    FROM public.bsl_speakers
    WHERE id = speaker_uuid;
    
    IF speaker_user_id IS NULL OR speaker_user_id = '00000000-0000-0000-0000-000000000000'::UUID THEN
        result := json_build_object(
            'success', false, 
    ...
    -- Step 12: Insert using INSERT...SELECT with explicit UUID variable
    -- Use the explicitly typed UUID variable instead of subquery
    ...
    SELECT 
        new_request_id::UUID,
        requester_uuid::UUID,
        speaker_user_id::UUID,  -- Use explicitly typed variable
        p_speaker_name::TEXT,
        p_requester_name::TEXT,
        p_requester_company::TEXT,
        p_requester_title::TEXT,
