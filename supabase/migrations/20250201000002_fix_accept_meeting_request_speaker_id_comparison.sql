-- Fix accept_meeting_request to correctly compare speaker_id
-- Issue: meeting_requests.speaker_id is UUID (user_id), not bsl_speakers.id
-- p_speaker_id is TEXT (bsl_speakers.id UUID as text)
-- We need to get the user_id from bsl_speakers and compare with meeting_requests.speaker_id
...
BEGIN
    ...
    -- Get the request
    -- meeting_requests.speaker_id is UUID (user_id), so compare with speaker_user_id
    SELECT * INTO request_record
    FROM meeting_requests
    WHERE id = p_request_id
      AND speaker_id = speaker_user_id  -- Compare UUID with UUID
    ...
    IF EXISTS (
        ...
        WHERE (
            (m.speaker_id = speaker_user_id OR m.requester_id = requester_user_id)
