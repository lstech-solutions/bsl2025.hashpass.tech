-- Fix LUKAS reward system: Add notifications for both users and ensure rewards are properly displayed
-- This migration updates accept_meeting_request to send LUKAS reward notifications to both speaker and requester
...
BEGIN
    ...
    -- Status is now 'confirmed' instead of 'tentative'
    -- IMPORTANT: Delete any existing entries for this meeting_id first to prevent duplicates
    -- This ensures each meeting only fills ONE slot per user
    DELETE FROM public.user_agenda_status
    WHERE user_id = speaker_user_id
      AND meeting_id = new_meeting_id;
    
    DELETE FROM public.user_agenda_status
    WHERE user_id = requester_user_id
      AND meeting_id = new_meeting_id;
    
    -- Speaker - Insert single entry for this meeting
    -- Since we deleted any existing entries above, this should always be a new insert
    -- But we include ON CONFLICT as a safety net in case of race conditions
    INSERT INTO public.user_agenda_status (
        ...
        new_meeting_id,
        'confirmed',  -- CHANGED from 'tentative' to 'confirmed'
        NULL  -- agenda_id must be NULL for meeting slots
    ...
    -- Requester - Insert single entry for this meeting
    -- Since we deleted any existing entries above, this should always be a new insert
    -- But we include ON CONFLICT as a safety net in case of race conditions
    INSERT INTO public.user_agenda_status (
        user_id,
    ...
    -- Rewards are now handled automatically by trigger_reward_on_meeting_confirmed
    -- This trigger fires AFTER the meeting is inserted, ensuring the meeting exists
    -- before rewards are given. This provides better separation of concerns and
    -- ensures rewards are given even if called from other code paths.
    
    -- Send notification to requester about meeting acceptance and LUKAS reward
    BEGIN
        PERFORM create_notification(
            requester_user_id,
            'meeting_accepted',
            'Meeting Request Accepted! 🎉',
            'Your meeting request has been accepted. Meeting scheduled for ' || to_char(p_slot_start_time, 'Mon DD, YYYY at HH24:MI') || '. You earned 1 LUKAS! 💎',
            p_request_id,
            speaker_user_id,
            false
        );
    ...
    -- Send notification to speaker about meeting acceptance and LUKAS reward
    BEGIN
        PERFORM create_notification(
            speaker_user_id,
            'meeting_accepted',
            'Meeting Scheduled! 🎉',
            'You accepted a meeting request. Meeting scheduled for ' || to_char(p_slot_start_time, 'Mon DD, YYYY at HH24:MI') || '. You earned 1 LUKAS! 💎',
            p_request_id,
            requester_user_id,
            false
        );
...
COMMENT ON FUNCTION accept_meeting_request IS 'Accepts a meeting request with slot selection. Sets status to confirmed, ensures duration_minutes is correctly set (15 minutes default). Rewards are automatically handled by trigger_reward_on_meeting_confirmed trigger. Sends LUKAS reward notifications to both users.';
